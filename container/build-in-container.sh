...
#
# Produces static binaries under ./.build/out/bin and caches sources/deps
# under ./.build/. The binaries are Linux/x86_64 executables — copy them to
...
# This script lives in container/, so the repo root is one level up.
...
mkdir -p "${ROOT}/.build/src" "${ROOT}/.build/obj" "${ROOT}/.build/prefix" "${ROOT}/.build/out" "${ROOT}/.build/ccache"

build_image() {
    echo "==> Building image ${IMAGE}"
    "${RUNTIME}" build --arch "${ARCH}" -t "${IMAGE}" -f "${ROOT}/container/Dockerfile" "${ROOT}"
}
...
mount_args() {
    local mounts=(
        ...
        "${ROOT}/.build/src:/work/src"
        "${ROOT}/.build/obj:/work/obj"
        "${ROOT}/.build/prefix:/work/prefix"
        "${ROOT}/.build/out:/work/out"
        "${ROOT}/.build/ccache:/ccache"
    )
    local m
...
ls -lh "${ROOT}/.build/out/bin" 2>/dev/null || echo "  (none produced)"
